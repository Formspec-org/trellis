...
- [ ] **O-5 — Posture-transition auditability enforced.** Custody-model and disclosure-profile changes are recorded as canonical events per Companion §10. **Retroactively reopened 2026-04-23** after design-doc audit (`thoughts/audit-2026-04-23-design-docs-vs-specs-and-code.md`) surfaced that `trellis-verify`'s `decode_transition_details` handles only custody-model transitions; a tampered `from_disclosure_profile` value currently passes verification. Re-close once the Rust verifier covers both transition kinds and a negative `tamper/` vector exercises a disclosure-profile mismatch. Tracking in [`../TODO.md`](../TODO.md) Stream 5. *(prior evidence, now incomplete: design brief `f94342b`; normative Companion posture-transition text + Appendix A.5 schemas in `8069062`; append posture-transition vectors `dbdfe0a`; tamper posture-transition vectors `814b2fe`; tamper-kind reconciliation `fd54232`.)*
