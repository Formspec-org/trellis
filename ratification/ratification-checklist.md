...
- [ ] **G-2 — Invariant coverage.** Every Phase 1 envelope invariant #1–#15 appears as normative MUST text in Core and is cross-referenced from at least one `TR-CORE-*` row. Byte-testable invariants are audited via the G-3 lint (`check_invariant_coverage`); non-byte-testable invariants (model-check / declaration-doc-check / spec-cross-ref paths) remain tracked here. *(evidence: G-3 lint + model-check registry now cover the byte-testable and model-check slices; see `thoughts/model-checks/evidence.toml` and `thoughts/model-checks/*.md`. Remaining open channels are declaration-doc-check, spec-cross-ref, projection-rebuild-drill, and manual-review.)*
- [ ] **G-3 — Byte-exact vectors.** ~50 test vectors under `fixtures/vectors/{append,verify,export,tamper,projection,shred}/` cover every byte-level claim. Every vector reproducible from Core prose alone. *(evidence: fixture system design `thoughts/specs/2026-04-18-trellis-g3-fixture-system-design.md`; 12-task scaffold plan `thoughts/specs/2026-04-18-trellis-g3-fixture-scaffold-plan.md`. 32 vectors landed across six op-dirs — append/1-9, verify/1-7, export/1, tamper/1-8, projection/1-5, shred/1-2. All G-3 coverage allowlists are closed (`_pending-projection-drills.toml` removed, `_pending-invariants.toml` removed, `_pending-matrix-rows.toml` removed, `_pending-model-checks.toml` emptied). Core gaps surfaced by G-3 authoring documented at `thoughts/specs/2026-04-18-trellis-core-gaps-surfaced-by-g3.md`. Residual gap: verify-vector breadth does not yet cover every §19 step — see `TODO.md` for steps 4.*, 5.d, 5.e, 6, 8 coverage follow-on. G-3 stays open until the residual verify steps are covered.)*
