...
## Current state (as of 2026-04-19, working tree atop HEAD = `b5aed8a`)
...
- **Lint:** green; 92/92 pytest. Lint-refactor commits 5-6 are implemented in the current working tree (R9/R10 O-5 extension registry + CDDL cross-ref; R11 O-4 declaration-doc static validator). `fixtures/vectors/_pending-invariants.toml` allowlist: **`pending_invariants = []`** (closed by `append/009`) + 51 `TR-*` rows remaining; `_pending-projection-drills.toml` 4 rows (TR-OP-006 removed by `projection/005`); `_pending-model-checks.toml` 8 rows awaiting G-4 evidence. Pre-merge vector-renumbering guard green (18 base prefixes preserved).
- **Fixture corpus:** 22 vectors across `append/{001,002,003,004,005,006,007,008,009}`, `projection/{001,002,003,004,005}`, `shred/{001,002}`, `tamper/{001,002,003,004,005,006}`. Reference `fixtures/declarations/ssdi-intake-triage/` O-4 artifact also landed, now with an event-registry stub for R11.
...
2. **`append/` residue batch (G-3)** — **done.**
      Closed by Wave 7 `append/009-signing-key-revocation` (TR-CORE-037 + TR-CORE-070). `pending_invariants = []` achieved.
...
5. **Expanded `tamper/` suite (G-3 close)** — **S** per case.
      Beyond first sig-flip: `tamper/005-chain-truncation` landed (`992fbc1`, TR-CORE-020, Core §19 step 4.h); `tamper/006-event-reorder` landed Wave 7 (TR-CORE-020, Core §19 step 4.h, `tamper_kind = event_reorder`); remaining cases: missing head, malformed COSE, wrong-scope, stale `prev_hash` residuals, registry-snapshot swap, checkpoint divergence.
...
- [ ] **Author remaining O-3 fixtures** — **S** (reduced from M). Priority order per Wave 4C handoff:
      ...
      (c) [x] `projection/005-watermark-staff-view-decision-binding` (TR-OP-006 + §17.4 Staff-View) — final O-3 breadth item landed Wave 7.
...
- **Wave 7 (in-tree, uncommitted as of this refresh):** Landed three Phase-1 fixtures closing residue batches. `append/009-signing-key-revocation` (TR-CORE-037 + TR-CORE-070) discharges the last two `pending_invariants` — Active→Revoked lifecycle (§8.2/§8.4/§8.5) paired with a committed §14.2 minimal domain registry + §14.3 `RegistryBinding` whose `registry_digest` byte-matches. `projection/005-watermark-staff-view-decision-binding` (TR-OP-006, Companion §17.4) — staff-view Watermark (`projection_schema_id = "trellis.staff-view.v1"`) plus committed §29.3 `StaffViewDecisionBinding` whose embedded `watermark` byte-matches the standalone Watermark; closes the final O-3 breadth item. `tamper/006-event-reorder` (TR-CORE-020) — second expanded-tamper case using byte-exact `append/001` + `append/005` events in swapped order; `tamper_kind = event_reorder` per `tamper/001`'s enum; distinct from `tamper/005`'s truncation variant. `pending_invariants = []`; TR-OP-006 dropped from both `_pending-invariants.toml` and `_pending-projection-drills.toml`. 92/92 pytest, check-specs green (warnings only), renumbering guard green (18 base prefixes).
- **Wave 6 (2 commits `992fbc1..6b20ef3`):** 4 parallel in-tree agents (no worktrees). Landed `tamper/005-chain-truncation` (TR-CORE-020, Core §19 step 4.h pin; first expanded-tamper case, `992fbc1`) and §9.4 HPKE-freshness decision memo recommending Option (a) amendment (`6b20ef3`, `thoughts/specs/2026-04-19-trellis-hpke-freshness-decision.md`). Stream D O-5 semi-formal review completed (verdict REQUEST CHANGES, 1 blocking-style finding + 1 nit + 6 notes; Finding 1 is the `tamper_kind` enum-name drift between `tamper/001` and `tamper/004`). `projection/005` authoring agent halted cleanly on a Phase-1 extension-registry spec blocker. 3 new spec-amendment tasks surfaced: `tamper_kind` reconcile, register `trellis.staff-view-decision-binding.v1`, land Core §9.4 amendment. 89/89 pytest; check-specs + renumbering guard green (18 base prefixes).
