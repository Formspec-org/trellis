...
One sequence, ordered by `Importance × Debt`. Size tags are scheduling
hints only — never inputs to priority. Each item lists its prerequisite
inline where it has one. Closed work is out of this file — see
...
clean (items 1-6, 12-17, 22, 25-29, 36, 38-40). Trellis-internal rows
(7-11, 18-21, 23-24, 30-35, 37) carry no parent counterpart by design — they
...
integrity artifact for ESIGN/UETA, closed Wave 22), **#5** (user-content
Attestation primitive — Trellis ADR 0010, parent PLN-0379 — the
sequenced sibling to #4), **#6** (c2pa-manifest adapter — layers the
certificate onto the presentation PDF), **#12** + **#13** (WOS-T4 + ADR
0073 shared-fixture residue), **#14** (identity attestation, supersedes
PLN-0310 → PLN-0381), ~~**#37**
...
ratified). Trellis items **16** (ADR 0066) and **21** (case ledger) may later
...
2. ~~**HPKE duplicate-ephemeral detection lint**~~ — **CLOSED Wave 17, 2026-04-27.**
   ...
   Core §9.4. See [`COMPLETED.md`](COMPLETED.md) Wave 17 entry.
   Item-number rebalance deferred at Wave 17 was executed Wave 22
   (signature-stack work promoted to slots #5..#8 + cascade).
...
4. ~~**Certificate-of-completion composition — execute per ADR 0007**~~ — **CLOSED Wave 22, 2026-04-28.**
   ...
   *Signature stack closure:* this lands the integrity artifact for
   ...
   The c2pa-manifest adapter at item #6 layers the certificate onto the
   presentation PDF in a downstream ADR slot.

5. **User-content Attestation primitive — author + execute Trellis ADR 0010**
    — **M–L**, **signature-stack**.
    *Coordinates parent **PLN-0379** (parent renumbered ADR reference
    ...
    parity per VISION §X. Parent TODO-STACK calls this out as one of
    three critical-path-leverage independent parallel work streams
    (alongside PLN-0384 WOS event-type taxonomy + PLN-0386 formspec-server
    cluster scaffold). Gate: none Trellis-internal.
...
6. **`c2pa-manifest` adapter — execute with ADR 0007** — **M**.
    *Co-lands with ADR 0007 reference-template work (closed Wave 22 —
    prerequisite met).* Implements
...
7. **Core §17 `idempotency_key` — Rust + fixtures + stores + verify (G-4
    scaffold catch-up)** — **L**.
    ...
    Pure tech-debt elimination: the spec pins behavior the runtime
    doesn't enforce, which is exactly the "silent disagreement between
    layers is architectural debt" frame.
...
8. **`did-key-view` adapter — execute with item #1 (ADR 0006)** — **XS**.
    *Co-lands with ADR 0006 `KeyEntry` migration (item #1 above, closed
    Wave 17 — prerequisite met).*
...
9. **Key-rotation grace-window semantics** — **XS**.
...
10. **Cadence subtypes beyond height-based** — **M**.
...
11. **O-4 ledger-replay lint rules 7–13** — **M**.
...
12. **WOS-T4 residue — shared cross-repo fixture bundle re-seeding** — **S**.
...
13. **ADR 0073 handoff residue — shared fixture alignment** — **S**.
    *Same prerequisite as #12 (shared cross-stack fixture bundle).*
...
14. **Identity attestation bundle shape** — **S**.
...
15. **Respondent Ledger ↔ Trellis `eventHash` MUST promotion** — **M**.
...
16. **ADR 0066 execution — amendment / supersession / rescission / correction**
    — **L**.
...
17. **ADR 0067 execution — statutory clocks** — **M**.
...
18. **`trellis.external_anchor.v1` priority interaction** — **S**.
...
20. **Disclosure-profile scope granularity (per-case)** — **M**.
...
21. **Case ledger + agency log semantic definitions** — **M**.
...
22. ~~**Interop sidecar reservation — execute per ADR 0008**~~ — **CLOSED Wave 20, 2026-04-27.**
...
25. **ADR 0068 execution — tenant in envelope and verifier** — **M**.
...
26. **ADR 0071 execution — `CaseOpenPin` and migration transitions**
    — **M–L**.
...
27. **ADR 0070 execution — `CommitAttemptFailure` ProvenanceKind** — **M**.
...
28. **ADR 0069 execution — chain timestamp-order verification** — **S–M**.
...
29. **ADR 0081 execution — content-addressed artifact identity custody integration**
    — **S**.
...
30. ~~**Wave 15 BLOCKER — Companion §A.5.2 reason-code corpus reconciliation**~~ — **CLOSED Wave 18, 2026-04-27.**
...
31. ~~**Wave 15 follow-up — R15 temporal-in-force enforcement**~~ — **CLOSED Wave 18, 2026-04-27.**
...
32. ~~**HPKE crate hardening (Wave 16 review follow-ups)**~~ — **CLOSED Wave 18, 2026-04-27.**
...
33. ~~**`trellis-store-postgres` review follow-ups (Wave 16)**~~ — **CLOSED Wave 18, 2026-04-27.**
...
34. ~~**ADR 0006 vector corpus completion**~~ — **CLOSED Wave 17, 2026-04-27.**
...
35. ~~**Reason-code parity lint**~~ — **CLOSED Wave 18, 2026-04-27.**
    ...
    sibling item #30's §A.5.2 table reconciliation.

36. **Stack-level security disclosure policy** — **S**, stack-coordination.
