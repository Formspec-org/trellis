...
| `EventPayload.extensions` | `trellis.supersedes-chain-id.v1` | 1 | Cross-chain supersession linkage per stack ADR 0066: payload carries `chain_id` and `checkpoint_hash` for the superseded chain (hash-anchored; verifier resolves linkage at bundle composition time). Map key in `EventPayload.extensions` is this registered identifier. Reject-if-unknown-at-version. |
