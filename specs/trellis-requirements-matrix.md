...
| TR-CORE-156 | core | — | Core §19 step 6d step 6: `signing_kid` MUST resolve to a Core §8 KeyEntry of class `signing` whose lifecycle state at `attested_at` is `Active`. `Rotating` is NOT admitted until the rotation-grace overlap window ratifies (ADR 0010 open question 4 / TODO item #5). Wrong class or wrong state flips `integrity_verified = false` with `user_content_attestation_key_not_active`. | Pins the key-state check at the time of attestation so an attestor cannot evade the lifecycle by signing under a Retired or Revoked kid; mirrors the ADR 0006 signing-class lifecycle gate enforced for canonical event signatures. | test-vector | — | Anchors ADR 0010 §"Verifier obligations" step 6. Fixtures: `tamper/034-uca-key-not-active`. |
