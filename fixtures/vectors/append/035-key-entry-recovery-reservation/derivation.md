# Derivation — `append/035-key-entry-recovery-reservation`

## Header

**What this vector exercises.** Phase-1 reservation positive for `kind = "recovery"` (ADR 0006 / Core §8.7.4).
...
to the §8.2 path); row 1 is a `KeyEntryNonSigning` row of `kind = "recovery"`
with the minimal `attributes` map per §8.7.2.

The genesis event in scope `test-key-entry-recovery-ledger` is signed by `issuer-001` and
...
and the `kind = "recovery"` declaration. Phase-2+ activation lifts the
warning per ADR 0006 §"Phase 2 evolution".

**Ledger scope.** `test-key-entry-recovery-ledger` is distinct from every other fixture's scope
...
| `ledger_scope` | bstr `"test-key-entry-recovery-ledger"` | §10.4 |
| `sequence` | `0` | §10.2 genesis |
| `authored_at` | `1745130800` | §12.1 narrative |
| `idempotency_key` | bstr `"idemp-append-035"` | §6.1 |
| `suite_id` | `1` (Phase-1 pin) | §7.1 |
| Non-signing class | `recovery` | ADR 0006 / Core §8.7.1 |
| Non-signing pubkey | `SHA-256("trellis-fixture-non-signing-pubkey-recovery")` (32 bytes) | fixture-only deterministic seed; the non-signing key does not sign anything in Phase 1 |
...
## Registry shape (load-bearing for 035-key-entry-recovery-reservation)
...
**Row 1 (`KeyEntryNonSigning`, kind = "recovery").** Five-field map header
`0xa5`; canonical-ordered keys `attributes`, `extensions`, `kid`,
`kind`, `suite_id`. Verifier dispatch on presence of `kind = "recovery"`
...
3. Row 1: `kind = "recovery"` → reserved-non-signing path;
...
| TR-CORE row | Where in 035-key-entry-recovery-reservation's bytes |
|---|---|
| TR-CORE-039 (KeyEntry taxonomy + dispatch) | The two-row `input-signing-key-registry.cbor` array exercises both dispatch paths in §8.7.3 step 1: row 0 has no `kind` (legacy); row 1 has `kind = "recovery"` (reserved non-signing). Both paths produce identical verification outcomes for the genesis event because the kid-resolution map is unioned across paths. |
| TR-CORE-047 (Phase-1 reservation acceptance + lint warning) | `expected-event.cbor` verifies under the two-row registry without `integrity_verified` flipping to false; the Phase-1 lint emits a warning naming `kind = "recovery"`. |
