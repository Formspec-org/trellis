...
**Refined by:** Parent [ADR 0073](../../../../thoughts/adr/0073-stack-case-initiation-and-intake-handoff.md) for case-boundary ownership. WOS owns governed case identity and `case.created`; Formspec emits intake handoff evidence; Trellis anchors and exports the evidence path.
...
- Expert Panel Review (`thoughts/research/2026-04-10-expert-panel-unified-ledger-review.md`)
- Technology Survey (`thoughts/research/2026-04-10-unified-ledger-technology-survey.md`)

---

## Part 7: Client-First Local Ledger (Architectural Reframe)

> **Added 2026-04-10.** This section reframes the entire architecture. The ledger does not start on the server. It starts in the browser.

### The insight

Every Formspec form already runs a Rust/WASM engine in the browser for offline-capable FEL evaluation, validation, branching, and repeat groups. The respondent's browser IS the primary compute environment. The ledger should originate there, not on the server.

The "unified ledger" is not a server-side database that the respondent exports. It is a client-side append-only log that the server receives. The respondent had the data before the server did. Sovereignty is literal, not architectural.

### Two synchronized copies

The unified ledger exists as two copies that sync:

| | Client Ledger | Server Ledger |
|---|---|---|
| **Where** | Browser OPFS/IndexedDB, extension storage, or native app SQLite | Postgres append-only table |
| **Who writes** | The respondent (form events) | Respondent (synced from client) + caseworkers + AI agents + system (governance events) |
| **Who signs** | Respondent's DID key (generated/stored in browser) | Respondent key (intake events) + platform key (governance events) |
| **What it stores** | Intake events only (session through submission) | Intake events (synced) + governance events (post-submission) |
| **Hash chain** | Client computes and signs | Server verifies client chain, then continues with governance events |
| **Offline** | Full capability (WASM engine evaluates locally) | N/A |
| **Encryption** | Respondent encrypts with own key before sync | Server adds tenant key wrapping for caseworker access |

### The Coprocessor transition

The intake handoff plus WOS-owned `case.created` event is where the client ledger hands off to the server ledger:

```
Client ledger (respondent's browser):
  session.started → draft.saved → ... → response.completed

     ↓ sync (HTTPS POST of new events since last sync)

Server verifies chain integrity, verifies respondent signatures

     ↓ Coprocessor

Formspec emits: IntakeHandoff (pins response, validation, and ledger head)
WOS accepts; Trellis appends: case.created (links intake chain to governance chain)

Server ledger continues:
  wos.transition.fired → wos.task.created → ... → wos.explanation.assembled
```

Before WOS accepts the intake handoff and emits `case.created`, the respondent owns the chain. After it, the platform extends the chain with governance events co-signed by the platform key. The respondent's local copy stays at the submission point (or continues if they file an RFI response or appeal).

### Browser-side technology

The Rust crates that do crypto on the server compile to WASM for the browser. One implementation, two deployment targets:

| Component | Technology | Notes |
|-----------|-----------|-------|
| Storage | OPFS (Origin Private File System) or IndexedDB | OPFS is faster, file-based, good for append-only. IndexedDB more universal. |
| Compute | formspec-engine (Rust/WASM, already exists) | FEL, validation, branching -- all run locally |
| Hash chain | `ring` SHA-256 → WASM | Same code as server |
| Signing | `ed25519-dalek` → WASM | Sign events with respondent's DID key |
| Encryption | `ring` AES-256-GCM → WASM | Encrypt sensitive fields before sync |
| Pedersen commitments | `curve25519-dalek` → WASM | Compute commitments client-side |
| Key storage | Web Crypto API (non-extractable keys) or extension secure storage | Private key never leaves the device |
| Sync | HTTP POST with chain verification | Send new events since last sync. Server verifies continuity. |

### The extension IS the wallet

The Formspec browser extension (or PWA, or native app) is simultaneously:

- A **form filler** (Formspec engine)
- A **local ledger** (append-only event store with hash chain)
- A **credential wallet** (DID key + VCs from ID.me/Login.gov)
- A **proof generator** (BBS+ selective disclosure over own events)
- A **chain verifier** (verify server's governance events against the chain)

The respondent does not need a separate wallet app. The tool they use to fill out the form IS the wallet that holds their identity, their case ledger, their encryption keys, and their proof generation capability.

### What this changes

1. **Data sovereignty is literal.** The respondent's data lives on their device first. The server gets a copy. The respondent does not "export" -- they already have the original.

2. **The respondent's key never leaves their device.** Key management is a client concern. The browser generates the DID key pair. Events are signed locally. Encryption happens locally. The server never sees the private key.

3. **Offline-first is the default.** The WASM engine evaluates offline. The local ledger records events offline. When connectivity returns, the chain syncs. A respondent on a rural Medicaid application with spotty cellular fills out the entire form, has every interaction recorded in their local ledger, and syncs when they reach a library with wifi.

4. **The sync protocol is simple.** POST new events since last sync. Server verifies: (a) the chain continues from where it left off, (b) respondent signatures are valid, (c) no events were inserted or reordered. If verification passes, server appends to its copy.

5. **Consistency verification is bidirectional.** The client periodically requests the server's latest signed tree head and a consistency proof against the client's last-known head. If the server added governance events, the client verifies they extend the chain correctly. If consistency fails, the respondent has cryptographic evidence of server tampering.

### Impact on ADR-0059 architecture

This reframe does not invalidate Parts 1-6 of the ADR. It clarifies WHERE the ledger originates (client, not server) and HOW the two copies relate. The encryption model, key hierarchy, Merkle proofs, event taxonomy, Temporal integration, and deployment tiers all apply to the server-side copy. The client-side copy uses the same cryptographic primitives compiled to WASM.

The Formspec Coprocessor spec must now define:

1. The sync protocol (client → server event transfer with chain verification)
2. The intake handoff and WOS-owned `case.created` transition from client-owned chain to server-extended chain
3. How subsequent client events (RFI responses, appeal submissions) append to the same chain
4. Consistency verification protocol (client verifying server's governance extensions)

### Expert panel input on Phase 1 with client-first model

The Phase 1 stack recommended by the expert panel (Postgres + Cloud KMS + OIDC + hash chain) remains valid for the server side. The client side adds:

| Component | Phase 1 | Later |
|-----------|---------|-------|
| Client storage | IndexedDB (universal) | OPFS (faster, when supported) |
| Client hash chain | SHA-256 via `ring` → WASM | Same |
| Client signing | HMAC with session key (simple) | Ed25519 DID signing |
| Client encryption | None (encrypt on server at sync) | Client-side AES-256-GCM before sync |
| Sync protocol | POST events, server verifies chain | Same, with consistency proofs |
| Offline capability | Already exists (WASM engine) | Same |
| Credential wallet | OIDC session token | DID + VCs |

Phase 1 client: a local IndexedDB event log with a hash chain, synced to the server over HTTPS. No DID keys, no client-side encryption, no BBS+ proofs. The WASM engine already runs offline -- the local ledger just records what it does. The trust model starts with the server verifying the client's chain, not the client verifying the server's.
