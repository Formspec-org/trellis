...
The `case.created` event is where the client ledger hands off to the server ledger:
...
Server appends: case.created (links intake chain to governance chain)

Server ledger continues:
  wos.transition.fired → wos.task.created → ... → wos.explanation.assembled
```

Before `case.created`, the respondent owns the chain. After it, the platform extends the chain with governance events co-signed by the platform key. The respondent's local copy stays at the submission point (or continues if they file an RFI response or appeal).
...
2. The `case.created` transition from client-owned chain to server-extended chain
