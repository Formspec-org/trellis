...
**Open items:** F6 status / `deprecated_at` lifecycle field check landed as `check_vector_lifecycle_fields()` in `scripts/check-specs.py`. **Renumbering pre-merge guard NOT landed** — ground-truth grep of `check-specs.py` confirms no renumber / branch-diff logic exists; the broader design-doc audit (`thoughts/audit-2026-04-23-design-docs-vs-specs-and-code.md` §5) is correct. Corrected 2026-04-23 after reconciliation; the renumbering guard is tracked as an open item under `TODO.md` Stream 9 (trigger-gated).
